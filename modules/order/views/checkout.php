        ...
        <div class="card-body">
          ...
          <form method="post" action="<?= BASE_URL ?>/?r=payment/payment/checkout" enctype="multipart/form-data">
            <div class="mb-2">
              ...
              <select name="shipping" class="form-select" id="shipping">
            ...
            <div class="mb-2" id="delivery_address_field">
            ...
            <div class="mb-2 d-none" id="pickup_branch_field">
              <label class="form-label">Pickup Branch</label>
              <select class="form-select" name="pickup_branch_id" id="pickup_branch_id">
                <option value="">Select a branch</option>
                <?php foreach ($pickupBranches as $branch): ?>
                  <option value="<?= htmlspecialchars((string) $branch['Branch_Id']) ?>">
                    <?= htmlspecialchars((string) $branch['Branch_Name']) ?> - <?= htmlspecialchars((string) $branch['Branch_Location']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
              <?php if (empty($pickupBranches)): ?>
                <div class="form-text text-danger">No branch currently has enough stock for every item in your cart.</div>
              <?php else: ?>
                <div class="form-text">Only branches with enough stock for every cart item are shown.</div>
              <?php endif; ?>
            </div>
            <div class="mb-3">
...
<script>
  window.addEventListener('load', function() {
    ...
    const destinationAddress = deliveryAddressField ? deliveryAddressField.querySelector('[name="destination_address"]') : null;
    const pickupBranchField = document.getElementById('pickup_branch_field');
    const pickupBranch = document.getElementById('pickup_branch_id');
    ...
    function syncShippingFields() {
      const isPickup = shipping && shipping.value === 'Pickup';
      deliveryAddressField.classList.toggle('d-none', isPickup);
      pickupBranchField.classList.toggle('d-none', !isPickup);
      if (destinationAddress) {
        destinationAddress.required = !isPickup;
      }
      if (pickupBranch) {
        pickupBranch.required = isPickup;
      }
    }
    cod && cod.addEventListener('change', syncPaymentFields);
    cashless && cashless.addEventListener('change', syncPaymentFields);
    shipping && shipping.addEventListener('change', syncShippingFields);
    syncPaymentFields();
    syncShippingFields();
  });
</script>
