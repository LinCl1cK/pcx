...
class ProductModel extends BaseModel {
    public function getAllProducts(?string $search = null, ?string $category = null): array {
        ...
        if ($search !== null && $search !== '') {
            $sql .= " AND (p.Prod_Name LIKE :search OR p.Prod_Brand LIKE :search)";
            $params['search'] = '%' . $search . '%';
        }

        if ($category !== null && $category !== '') {
            $sql .= " AND (c.Cat_Name = :categoryName OR c.Cat_Id = :categoryId)";
            $params['categoryName'] = $category;
            $params['categoryId'] = $category;
        }

        $sql .= " ORDER BY p.Prod_CreatedAt DESC";
    ...
    public function getOrCreateCartId(string $customerId): string {
